...
import dev.itsrealperson.permadeath.nms.v1_21_R3.utils.SpawnUtils;
...
public class QuantumReactor {
    ...
    private static final Map<UUID, Guardian> beamEntities = new HashMap<>();
    ...
    public static ArmorStand spawn(Location loc, Plugin plugin) {
        ...
        // Emisor de láser invisible
        Guardian beamSource = (Guardian) loc.getWorld().spawnEntity(loc.clone().add(0, 1.5, 0), EntityType.GUARDIAN, CreatureSpawnEvent.SpawnReason.CUSTOM);
        beamSource.setInvisible(true);
        beamSource.setSilent(true);
        beamSource.setAI(false);
        beamSource.setInvulnerable(true);
        beamSource.setGravity(false);
        beamEntities.put(reactor.getUniqueId(), beamSource);

        // Configuración Visual
        reactor.setCustomName("§b§l§k!!§r §b§lQUANTUM REACTOR§r §b§l§k!!");
        ...
        Runnable reactorTask = new Runnable() {
            ...
            public void run() {
                if (reactor.isDead() || !reactor.isValid()) {
                    ...
                    Guardian g = beamEntities.remove(reactor.getUniqueId());
                    if (g != null) g.remove();
                    return;
                }
                ...
                // El emisor del láser sigue al reactor
                Guardian g = beamEntities.get(reactor.getUniqueId());
                if (g != null) g.teleport(reactor.getLocation().add(0, 1.5, 0));
                ...
                if (target != null) {
                    // Láser Visual (Packet)
                    if (g != null) dev.itsrealperson.permadeath.util.PacketManager.sendGuardianBeam(g.getEntityId(), target.getEntityId(), g.getLocation());

                    // Daño progresivo
                    if (angle % 2.0 < 0.2) { // Ataca cada cierto tiempo
                        target.damage(4.0, reactor);
                        dev.itsrealperson.permadeath.util.PacketManager.sendFakeLightning(target.getLocation());
                        target.getWorld().playSound(target.getLocation(), Sound.BLOCK_BEACON_POWER_SELECT, 1.5f, 2.0f);
                    }
                } else if (g != null) {
                    // Quitar el láser si no hay objetivo (Target ID 0 o el mismo Guardian suele apagarlo)
                    dev.itsrealperson.permadeath.util.PacketManager.sendGuardianBeam(g.getEntityId(), g.getEntityId(), g.getLocation());
                }
            }
        };
