        ...
        for (int i = 0; i < ACCESSORY_SLOTS.length; i++) {
            ...
            if (item != null) {
                if (item.isSimilar(filler)) {
                    ...
                    // Si es uno de los primeros 3 slots y tiene su placeholder, no lo quitamos
                }
            }
        }
        ...
        try {
            ...
            for (int i = 0; i < ACCESSORY_SLOTS.length; i++) {
                ...
                if (item != null && item.getType().name().contains("STAINED_GLASS_PANE") && item.hasItemMeta()) {
                    ...
                    if (name.contains("Slot de Orbe") || name.contains("Slot de Medalla") || name.contains("Slot de Reliquia")) {
                        item = null;
                    }
                }
            ...
            dataOutput.writeInt(accessoryItems.length);
            for (ItemStack item : accessoryItems) {
