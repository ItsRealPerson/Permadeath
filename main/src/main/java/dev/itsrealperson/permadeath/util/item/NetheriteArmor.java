...
public final class NetheriteArmor implements Listener {
    ...
    public static Double getAvailableMaxHealth(Player p) {
        ...
        ItemStack[] accessories = dev.itsrealperson.permadeath.util.inventory.AccessoryInventory.load(p);
        ...
        // Extra HP from Player Data (Admins/Events)
        dev.itsrealperson.permadeath.data.PlayerDataManager manager = new dev.itsrealperson.permadeath.data.PlayerDataManager(p.getName(), Main.getInstance());
        maxHealth += (double) manager.getExtraHP();
