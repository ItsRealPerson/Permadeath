package dev.itsrealperson.permadeath.task;
...
import dev.itsrealperson.permadeath.end.demon.DemonCurrentAttack;
import dev.itsrealperson.permadeath.end.demon.DemonPhase;
...
public class EndTask extends BukkitRunnable {
    ...
    private void tickRandomLighting() {
        ...
        Location lightningLoc = new Location(main.endWorld, x, y, z);
        dev.itsrealperson.permadeath.util.PacketManager.sendFakeLightning(lightningLoc);
        
        if (random.nextBoolean()) {
            dev.itsrealperson.permadeath.util.PacketManager.sendVisualExplosion(lightningLoc, 2.0f);
        }
    }
        ...
        if (lightingRain) {
            if (lightingDuration >= 1) {
                ...
                for (Player all : main.endWorld.getPlayers()) {
                    ...
                    dev.itsrealperson.permadeath.util.PacketManager.sendFakeLightning(all.getLocation());
    ...
    public void chooseAnAttack() {
        ...
        if (ran < 15) {
            ...
            for (Player p : main.endWorld.getPlayers()) {
                dev.itsrealperson.permadeath.util.PacketManager.sendCameraShake(p.getLocation(), 20.0f);
            }
            ...
            for (Player p : main.endWorld.getPlayers()) {
                dev.itsrealperson.permadeath.util.PacketManager.sendCameraShake(p.getLocation(), 15.0f);
            }
        } else if (ran < 45) {
